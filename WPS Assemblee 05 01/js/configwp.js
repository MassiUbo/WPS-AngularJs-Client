		...
		for (var indexOutput in response.processOffering.process.outputs)	{
			for (var property in response.processOffering.process.outputs[indexOutput]) {
				...
				if ((property == "literalData")||(property == "complexData")){
					...
					newdiv.innerHTML += 'Nom fichier: <input type="text" id="fileName'+ indexOutput +'" name="fileName" style="width: 100px; height: 15px;" value="' + processDescription.processOffering.process.outputs[indexOutput].title +  '" />';
					newdiv.innerHTML += "<br />"
					outputs += '\n';
					nbo = indexOutput;
				}
