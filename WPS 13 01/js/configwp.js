...
function executeCallback (data, url) {
	...
	selectedIndex = selectedIndex -1 ;
	 ...
	 if (isWeb[selectedIndex][0] == false) {
	    ...
	    $("textarea#processDescriptionText").val(JSON.stringify(wpsResponse)); 
	}
	 // si non on excute sur le web 
	 else{
		$("textarea#processDescriptionText").val(response); 
		// window.open(url);
	 }
		...
		for (var indexOutput in response.processOffering.process.outputs)	{
			for (var property in response.processOffering.process.outputs[indexOutput]) {
				...
				if ((property == "literalData")||(property == "complexData")){
					...
					newdiv.innerHTML += " <b>Resultat XML</b>  <input type='checkbox' checked='checked' name='3" + indexOutput + "' " +"' id='3" + indexOutput + "' onclick='checker(3" + indexOutput + ",4" + indexOutput + ");' />" + " <b>Résultat JSON<b/> <input type='checkbox' name='4" + indexOutput + "' id='4" + indexOutput + "' onclick='checker(4" + indexOutput + " ,3" + indexOutput + ");' />";
