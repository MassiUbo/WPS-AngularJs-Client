...
myApp.controller("myController", function ($scope, $http) {   // controller 
	...
	/* $scope.recup21= function(process){
		$scope.getDescriptionProcess(process.process)
		setTimeout( $scope.getData2, 3000);
		//setTimeout( $scope.recup2(process),4000)
	 } */
	...
	$scope.ExecuteProcess = function (id) {
		...
		if ( texRef == 0 ) {
		 {  
			...
			$http({ method: 'GET' ,url :"http://portail.indigeo.fr/geoserver/LETG-BREST/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=LETG-BREST%3AREF_VOUG&maxFeatures=50&outputFormat=application%2Fjson" 
	...
	$scope.activer1 = function () {
		...
		for (i = 0; i < pp.length; i++) {
			if (pp[i].LiteralData)
				document.getElementById('myInputs1' + i).disabled = false
		}
		//document.getElementById('minint').disabled = false;
	...
	$scope.desactiver1 = function () {
		for (i = 0; i < pp.length; i++) {
			if (pp[i].LiteralData)
				document.getElementById('myInputs1' + i).disabled = true
		}
		//document.getElementById('inputint').disabled=true;
		//document.getElementById('minint').disabled = true;
		//document.getElementById('maxint').disabled = true;
	...
	$scope.changestep1 = function () {
		...
		for (i = 0; i < pp.length; i++) {
			if (pp[i].LiteralData)
				document.getElementById('myInputs1' + i).step = s
		}
		displayinfo("configuration pris en compte !")
